...
{
    [TestFixture]
  public  class RejectUserInTrip_Should
    {
        [Test]
        public void Have_HttpPost_Attribute()
        {
            // Arrange
            var method = typeof(DashboardController).GetMethod(nameof(DashboardController.RejectUserInTrip));
        ...
        public void Render_TripSharedByUserInfoPartial_WithCorrectDataFromTheService()
        {
            ...
            mockedUserDashboardService.Setup(x => x.RejectUserToJoinTrip(userId, tripId))
                .Returns(data);

            // Act and Assert
            controller.WithCallTo(x => x.RejectUserInTrip(userId, tripId))
